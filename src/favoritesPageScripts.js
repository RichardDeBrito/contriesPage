...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (favorites.length === 0) {
        document.getElementById("container-contries").innerHTML = '<p>Nenhum favorito salvo.</p>';
        return;
    };
    ...
    const populationList = favorites.map(item => item.flags);
    const imgList = favorites.map(item => item.flags);

    createCardsContries(nameList, capitalList, regionList, populationList, imgList, favorites);
});